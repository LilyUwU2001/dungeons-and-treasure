...
def cast_teleport(*args, **kwargs):
    ...
    for entity in entities:
        if entity.x == target_x and entity.y == target_y and entity.fighter:
            rndx = 0
            rndy = 0
            while (game_map.is_blocked(rndx, rndy)):
                rndx = randint(1, game_map.width-1)
                rndy = randint(1, game_map.height-1)
            entity.x = rndx
            entity.y = rndy

            results.append({'fov_recompute': True, 'consumed': True, 'message': Message('The body of the {0} vanishes, as he appears in a different space on this floor!'.format(entity.name), libtcod.light_green)})
